...
class ObservationEncoder(torch.nn.Module):
  ...
  def initialize(
    ...
    Args:
      ...
      observation_normalizer: Flat observation normalizer, optional.
...
class UnflatObservationEncoder(torch.nn.Module):
  ...
  def initialize(
    ...
    Args:
      ...
      observation_normalizer: Unflat observation normalizer, optional.
    ...
    observation_space_flat = utils.flatten_space(self.observation_space)
  ...
  def forward(self, observations: dict[str, torch.Tensor], /) -> torch.Tensor:
    ...
  @T.overload
  def forward(self, *inputs: dict[str, torch.Tensor]) -> T.NoReturn:
    ...
  def forward(self, *inputs: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    observations_flat = utils.flatten_tensors(self.observation_space, observations)  # [batch, obs]
    return observations_flat


class UnflatObservationActionEncoder(torch.nn.Module):
  ...
  def initialize(
    ...
    observation_space_flat = utils.flatten_space(self.observation_space)
    action_space_flat = utils.flatten_space(self.action_space)
    observation_size = observation_space_flat.shape[0]
  ...
  def forward(self, *inputs: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    if self.observation_normalizer:
      observations = self.observation_normalizer(observations)  # {key: [batch, ...]}
    observations_flat = utils.flatten_tensors(self.observation_space, observations)  # [batch, obs]
    actions_flat = utils.flatten_tensors(self.action_space, actions)  # [batch, act]
